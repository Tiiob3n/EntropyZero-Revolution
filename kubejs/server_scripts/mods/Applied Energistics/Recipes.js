...
ServerEvents.recipes(entropy => {
    entropy.remove({ id: 'aeinfinitybooster:infinity_card' })
    entropy.shaped('aeinfinitybooster:infinity_card', ['EBE', 'BUB', 'NNN'], {
        U: '#c:ingots/sky_steel',
        B: 'ae2:wireless_booster',
        E: '#c:ingots/sky_steel',
        N: 'minecraft:netherite_ingot'
    }).id('entropy:aeinfinitybooster/infinity_card')

    entropy.shaped('kubejs:universal_press', ['FPF', 'CSL', 'FEF'], {
    ...
    function universalPress(input, output, id) {
        ...
        ).id(`entropy:inscriber/universal_press/${id}`)
    }
    ...
    function createCrystalAssemblerRecipe(output, inputs, id, fluid) {
        ...
        entropy.custom(recipe).id(`entropy:crystal_assembler/${id}`);
    }
    ...
    colors.forEach(color => {
        entropy.shapeless(`4x ae2:${color}_covered_cable`, [`ae2:${color}_covered_dense_cable`]).id(`entropy:ae2/${color}_dense_covered_to_normal`);
        entropy.shapeless(`4x ae2:${color}_smart_cable`, [`ae2:${color}_smart_dense_cable`]).id(`entropy:ae2/${color}_smart_dense_to_normal`);
        entropy.shapeless(`ae2:${color}_covered_dense_cable`, [`4x ae2:${color}_covered_cable`]).id(`entropy:ae2/${color}_covered_to_dense`);
        entropy.shapeless(`ae2:${color}_smart_dense_cable`, [`4x ae2:${color}_smart_cable`]).id(`entropy:ae2/${color}_smart_to_dense`);
    });
